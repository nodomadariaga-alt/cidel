...
export default function FloatingSocials({ whatsappNumber }: Props) {
    ...
    useEffect(() => {
        const handleScroll = () => {
            // El hero mide aprox 400px. Mostramos la barra después de bajar 300px.
    ...
    return (
        <div
            className={`fixed right-0 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3 p-2 bg-white/90 backdrop-blur-md border border-r-0 border-gray-200 rounded-l-2xl shadow-lg transition-all duration-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12 pointer-events-none'
                }`}
        >
            <a
